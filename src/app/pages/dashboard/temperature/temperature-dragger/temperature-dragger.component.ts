...
export class TemperatureDraggerComponent implements AfterViewInit, OnChanges {
    ...
    private invalidate(): void {
        this.bottomAngleRad = TemperatureDraggerComponent.toRad(
            this.bottomAngle
        );
        this.calculateVars();
    ...
    private calculateVars() {
        ...
        if (circleFactor > svgAreaFactor) {
            ...
            this.radius =
                (svgHeight - 2 * thumbMargin) / (1 + Math.cos(halfAngle));
        }
    ...
    private invalidateGradientArcs() {
        ...
        const angleStep =
            (2 * Math.PI - this.bottomAngleRad) / this.colors.length;
    ...
    private recalculateValue(event, allowJumping = false) {
        if (this.isMouseDown && !this.off) {
            ...
            const center = {
                ...
                y:
                    rect.top +
                    (this.translateYValue + this.radius) * this.scaleFactor,
            };
            ...
            if (
                this.value !== value &&
                (allowJumping ||
                    Math.abs(relativeValue - previousRelativeValue) <
                        this.maxLeap)
            ) {
                this.value = value;
    ...
    private toValueNumber(factor) {
        return (
            Math.round((factor * (this.max - this.min)) / this.step) *
                this.step +
            this.min
        );
    }
